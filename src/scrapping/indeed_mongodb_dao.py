...
class IndeedMongodbDao:
    ...
    def insert_data_duplictaed_bulk(self, data):
        try:
            self.collection_duplicate_data.insert_many(data)
        except BulkWriteError as bwe:
            print(bwe.details)
            print(bwe.details['writeErrors'])
            raise
    ...
    def updjate_data_live(self, id, url, title, name, address, publication_date, salary, description, localisation,
                          contract_type, job):
        try:
    ...
    def insert_data(self, url, title, name, address, publication_date, salary, description, localisation, contract_type, job):
        try:
            if url == "":
                raise Exception('url cannot be empty {}'.format(url))
    ...
    def get_data_by_url(self, url):
        return self.collection.find({"url": url})[0]
    ...
    def get_all_duplicate(self):
        return self.collection.aggregate(
            [{"$group": {"_id": "$url", "count": {"$sum": 1}}}, {"$match": {"_id": {"$ne": None}, "count": {"$gt": 1}}},
             {"$project": {"u": "$_id", "_id": 0}}])
