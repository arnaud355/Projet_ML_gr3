...
class IndeedPaser:
    ...
    def _local_parse_page(self, item_link, localisation, job):
        try:
            if self.dao.url_exist(item_link) == True:
                item_data = self.dao.get_data_by_url(item_link)
                title = item_data["titre"]
                name = item_data["nom_entreprise"]
                address = item_data["adresse"]
                date = item_data["date_de_publication"]
                salaire = item_data["salaire"]
                description = item_data["description"]
                localisation = item_data["localisation"]
                contract_type = item_data["type_de_contrat"]
                self.dao.updjate_data_live(item_data["_id"], item_link, title, name, address, date, salaire,
                                           description, localisation, contract_type, job)
                ...
                title, name, address, date, salaire, description, contract_type = self.indeed_item_parser.parse(
                    item_link)
                ...
                if (item_link != np.nan) & (title != np.nan) & (name != np.nan) & (address != np.nan) & (
                        description != np.nan):
    ...
    def update_data(self):
        ...
        for index_i, item in enumerate(items):
            ...
            try:
                ...
                title, name, address, date, salaire, description, contract_type = self.indeed_item_parser.parse(
                    item["url"])
                if (item["url"] != np.nan) & (title != np.nan) & (name != np.nan) & (address != np.nan) & (
                        description != np.nan):
                    self.dao.update_data(item["_id"], item["url"], title, name, address, date, salaire, description,
                                         contract_type)
                ...
                for location in self.locations:
                    ...
                    for page_index in random.sample(range(0, pages_count), pages_count):
                        ...
                        for index_i, link in enumerate(items):
                            self._local_parse_page(link, location, job)
